...
class TriBBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(TriBBoxHead, self).__init__(init_cfg)
        assert with_cls or with_reg
        self.with_avg_pool = with_avg_pool
        self.with_cls = with_cls
        self.with_reg = with_reg
        self.roi_feat_size = _pair(roi_feat_size)
        self.roi_feat_size_1 = tuple((3, 11))  # 新增
        self.roi_feat_area = self.roi_feat_size[0] * self.roi_feat_size[1] + 2 * self.roi_feat_size_1[0] * self.roi_feat_size_1[1]  # 新增
        # self.roi_feat_area = self.roi_feat_size[0] * self.roi_feat_size[1]  # 原始
        ...
        if self.with_avg_pool:
            ...
            self.avg_pool_1 = nn.AvgPool2d(self.roi_feat_size_1)  # 新增
            self.avg_pool_2 = nn.AvgPool2d(self.roi_feat_size_2)  # 新增
        else:
    ...
    def forward(self, x):
        if self.with_avg_pool:
            if x.numel() > 0:
                features = x  # 新增
                x = self.avg_pool(features)  # 原始x = self.avg_pool(x)
                x_1 = self.avg_pool_1(features)  # 新增
                x_2 = self.avg_pool_2(features)  # 新增
                x = x.view(x.size(0), -1)
                x_1 = x_1.view(x_1.size(0), -1)  # 新增
                x_2 = x_2.view(x_2.size(0), -1)  # 新增
                x = torch.cat((x, x_1, x_2), 0)  # 新增
            else:
