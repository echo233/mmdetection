...
@DATASETS.register_module(name='FewShotVOCDataset',force=True)
class FewShotVOCDataset(VOCDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        for shot in (1, 2, 3, 5, 10):
            ...
            split_dir = '/liu/code/d2l-zh/pycharm-local/FADI-Main/data/few_shot_voc_split'  # 新增
            for cls in classnames:  # 这个for循环是类别和对应的图像，用fileids字典收集
            ...
            for cls, fileids_ in fileids.items():  # dicts_表示xml解析后，小样本所需注释文件
                dicts_ = []
                for fileid in fileids_:
                    year = '2012' if '_' in fileid else '2007'
                    dirname = os.path.join('/liu/code/d2l-zh/pycharm-local/FADI-Main/data/VOCdevkit/',
                                           'VOC{}'.format(year))
                ...
                dicts.extend(dicts_) # {file_name，image_id，height，width，idx，annotations[category_id,bbox]}
        ...
        return data_infos # 'id': '2008_006761', 'filename': ，'height': ，'width': ，'annotations': {'bboxes': 'labels': }，长度是总的shot数，比如10shot就有10shot*20类=200
...
@DATASETS.register_module(name='FewShotVOCTestDataset',force=True)
